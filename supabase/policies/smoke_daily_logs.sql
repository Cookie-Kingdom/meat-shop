...
-- smoke_daily_logs's SELECT policy lands when the card that owns its view is written.
--
...
alter table public.smoke_daily_logs enable row level security;
revoke all on public.smoke_daily_logs from anon, authenticated;
