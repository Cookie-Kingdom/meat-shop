...
-- smoke_fee_tiers's SELECT policy lands when the card that owns its view is written.
--
...
alter table public.smoke_fee_tiers enable row level security;
revoke all on public.smoke_fee_tiers from anon, authenticated;
