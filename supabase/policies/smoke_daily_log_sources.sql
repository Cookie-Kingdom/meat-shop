-- smoke_daily_log_sources — RLS posture.
--
...
-- smoke_daily_log_sources's SELECT policy lands when the card that owns its view is written.
--
...
alter table public.smoke_daily_log_sources enable row level security;
revoke all on public.smoke_daily_log_sources from anon, authenticated;
