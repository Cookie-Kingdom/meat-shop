...
-- REPORT_CLOSED on a branch_expenses insert against a CLOSED report unless an APPROVED
-- DAILY_REPORT unlock_requests row with expires_at > now() exists, and an approval leaves the
-- report CLOSED (PLAN-sales.md T1 §3; ref-08-unlock/PLAN-unlock.md Finding 1). TC-84 and
-- TC-85's last two cases fail until lanes C and H merge; nothing else here depends on them.
--
...
begin
  ...
  --------------------------------------------------------------------------------- TC-85
  -- A day ten back and OPEN: today's report at A was closed in TC-84, so daily_reports_one_open
  ...
  insert into unlock_requests (target_type, target_id, requested_by, reason, status,
       ...
       values ('DAILY_REPORT', v_rep_old, v_adm_a, 'ลงค่าใช้จ่ายย้อนหลัง', 'APPROVED',
  ...
  assert v_err is null,
    format('TC-85: a CLOSED day ten back under a live unlock refused an expense, got [%s]', v_err);
  select count(*) into v_n from branch_expenses where daily_report_id = v_rep_old;
  assert v_n = 1, format('TC-85: the unlocked day holds %s expense rows, expected 1', v_n);
  ...
  v_err := null;
  begin
    perform fn_record_branch_expense(gen_random_uuid(), v_rep_old, 'ค่าแก๊ส', 60.00, 'สมชาย');
  exception when others then v_err := sqlerrm;
  end;
  assert v_err like '%REPORT_CLOSED%',
    format('TC-85: a CLOSED day with an expired unlock took an expense, got [%s]', coalesce(v_err, 'no error at all'));
  select count(*) into v_n from branch_expenses where daily_report_id = v_rep_old;
  assert v_n = 1, format('TC-85: the expired unlock let %s expense rows onto the day, expected 1', v_n);
