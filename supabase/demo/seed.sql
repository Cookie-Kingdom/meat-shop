...
begin
  ...
  perform fn_record_lot_receipt(gen_random_uuid(), v_lotA, d - 9, 98.00, 96.50);
  ...
  -- Lot B: received, three days logged, still smoking.
  perform fn_record_lot_receipt(gen_random_uuid(), v_lotB, d - 4, 98.00, 96.50);
  perform fn_upsert_smoke_daily_log(gen_random_uuid(), v_lotB, d - 4,
    jsonb_build_array(jsonb_build_object('lot_id', v_lotB, 'input_weight_kg', 32.00)));
  perform fn_upsert_smoke_daily_log(gen_random_uuid(), v_lotB, d - 3,
    jsonb_build_array(jsonb_build_object('lot_id', v_lotB, 'input_weight_kg', 32.00)));
  perform fn_upsert_smoke_daily_log(gen_random_uuid(), v_lotB, d - 2,
    jsonb_build_array(jsonb_build_object('lot_id', v_lotB, 'input_weight_kg', 32.50)));
